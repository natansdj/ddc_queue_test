...
func (c *Consumer) Consume(handler func(delivery amqp.Delivery)) error {
	...
	c.RabbitMQ.log.Info().Msg("handle: deliveries channel starting")
	...
	c.RabbitMQ.log.Info().Msg("handle: deliveries channel closed")
	c.done <- nil
	return nil
}
...
func (c *Consumer) Get(handler func(delivery amqp.Delivery)) error {
	...
	if ok {
		c.RabbitMQ.log.Debug().Msg("Message received")
		handler(message)
	} else {
		c.RabbitMQ.log.Debug().Msg("No message received")
	}
...
func (c *Consumer) Shutdown() error {
	...
	defer c.RabbitMQ.log.Info().Msg("Consumer shutdown OK")
	c.RabbitMQ.log.Info().Msg("Waiting for Consumer handler to exit")
