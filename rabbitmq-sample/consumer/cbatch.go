...
func main() {
	...
	utils.ShowUsageAndStatusConsumer("Consumer Batch")
	...
	// Declare and register a consumer
	cns2 := cony.NewConsumer(
		que,
		cony.Qos(10),
		//cony.AutoAck(),
		cony.Tag("OrderConsumer2"),
	)
	cli.Consume(cns2)
	...
	go func() {
		...
		for cli.Loop() {
			...
			case msg := <-cns2.Deliveries():
				//time.Sleep(100 * time.Millisecond) // naive backoff
				log.Printf("2Received body: %q\n", msg.Body)

				//Unmarshal
				var ipt map[string]string
				err := json.Unmarshal(msg.Body, &ipt)
				if err != nil {
					log.Printf("2FATAL Unmarshal : %s", err.Error())
					if err = msg.Nack(true, false); err != nil {
						log.Printf("2FATAL msg.Nack : %s", err.Error())
					}
					continue //continue loop
				}
